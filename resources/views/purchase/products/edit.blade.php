...
<form name="form-item" method="PUT" action="{{ route('products.update', ['id' => $item->id ]) }}" enctype="multipart/form-data">
  ...
  <div class="form-group">
    ...
    <input type="text" required class="form-control" name="price"  value="{{ $item->price }}" />
  </div>
  <div class="form-group">
	...
	<div id="feature_image">
        @php $images = explode("|", $item->image); @endphp
  ...
  <div class="form-group">
    ...
    <input type="url" pattern="https?://.+" required class="form-control" name="video" value="{{ $item->video }}" />
  </div>
  <div class="form-group">
        <iframe class="video" width="560" height="315" src="{{$item->video}}" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
  </div>
  <div class="form-group">
  ...
  <div class="form-group">
    ...
    <select name="cate_id" class="form-control">
        @foreach ($cates as $cate)
            <option @if ($cate->id == $item->cate_id) selected @endif value="{{ $cate->id }}"> {{ $cate->name }} </option>
        @endforeach
    </select>
  </div>
...
var processSelectedFile = function(file, feature_image){
    ...
    if($.isArray(file)) {
        for(var i =0; i<file.length; i++) {
            $('#feature_image').html('<img src="'+'storage/'+file[i].path+'" />')
            arr.push(file[i].path)
        }
    } else {
        $('#feature_image').html('<img src="'+'storage/'+file.path+'" />')
        arr.push(file.path)
    }
    $('[name="image"]').val(arr.join('|'))
    //$('[name="image[]"]').val(arr)
